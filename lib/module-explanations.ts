// This file contains explanations for each module in the RWA calculator

export function getModuleExplanation(moduleId) {
  if (moduleId === "pd") {
    return {
      title: "Point-in-Time PD Module",
      description: "Calculates the probability of default based on current economic conditions",
      purpose: "To estimate the likelihood of a counterparty defaulting within the next 12 months",
      businessContext: "Used for short-term credit decisions and pricing",
      regulatoryContext: "Required input for TTC PD calculation and RWA calculation",
      formula: ["PIT PD = f(Financial Ratios, Market Indicators, Current Economic Conditions)"],
      formulaExplanation:
        "The Point-in-Time PD is calculated using a combination of financial ratios, market indicators, and current economic conditions. It represents the default probability under current conditions.",
      examples: [
        "A counterparty with strong financials but in a cyclical industry during an economic downturn might have a higher PIT PD than its long-term average.",
        "A counterparty with stable cash flows in a non-cyclical industry might have similar PIT and TTC PDs.",
      ],
      keyConsiderations: [
        "PIT PD is more volatile than TTC PD as it reflects current economic conditions",
        "During economic booms, PIT PD is typically lower than TTC PD",
        "During economic downturns, PIT PD is typically higher than TTC PD",
        "PIT PD is used as an input to calculate TTC PD",
      ],
      inputParameters: [
        {
    ...
    return {
      ...
      purpose:
        "To provide a stable, long-term view of default risk that is less sensitive to current economic conditions",
      businessContext: "Used for capital planning, stress testing, and long-term risk assessment",
      regulatoryContext: "Required for regulatory capital calculations under Basel III",
      formula: ["TTC PD = PIT PD × (1 + (0.5 - Economic Index) × Cyclicality × 2) × 0.7 + Long-Term Average × 0.3"],
      formulaExplanation:
        "The TTC PD adjusts the PIT PD to account for economic cycles. When the economy is strong (Economic Index > 0.5), TTC PD is higher than PIT PD. When the economy is weak (Economic Index < 0.5), TTC PD is lower than PIT PD.",
      examples: [
      ...
      keyConsiderations: [
        "TTC PD is more stable than PIT PD across economic cycles",
        "TTC PD is used for regulatory capital calculations to reduce procyclicality",
        "The adjustment depends on the current position in the economic cycle and the cyclicality of the industry",
        "TTC PD is blended with long-term average default rates for stability",
      ],
      inputParameters: [
        {
          name: "Point-in-Time PD",
